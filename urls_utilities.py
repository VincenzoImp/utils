...
def get_url_normalized(apex=None, path=None):
    ...
    Examples
    --------
    >>> get_url_normalized('www.google.com', 'search')
    'www.google.com/search'
    >>> get_url_normalized('www.google.com', None)
    'www.google.com'
    """
    if not isinstance(path, str):
        if not isinstance(apex, str):
...
def add_url_info(df, url_column, tlds=tlds, prefix=None, suffix=None):
    ...
    >>> add_url_info(df, 'url')
    >>> df
                                  url  url_normalized  apex  second  first    tld    path  extension
    0          https://www.google.com  www.google.com   www  google    com   None    None       None
    1           http://www.google.com  www.google.com   www  google    com   None    None       None
    2                  www.google.com  www.google.com   www  google    com   None    None       None
    """
    df[f'{prefix if isinstance(prefix, str) else ""}_{url_column}_normalized_{suffix if isinstance(suffix, str) else ""}'],
    df[f'{prefix if isinstance(prefix, str) else ""}_apex_{suffix if isinstance(suffix, str) else ""}'],
    df[f'{prefix if isinstance(prefix, str) else ""}_second_{suffix if isinstance(suffix, str) else ""}'],
    df[f'{prefix if isinstance(prefix, str) else ""}_first_{suffix if isinstance(suffix, str) else ""}'],
    df[f'{prefix if isinstance(prefix, str) else ""}_tld_{suffix if isinstance(suffix, str) else ""}'],
    df[f'{prefix if isinstance(prefix, str) else ""}_path_{suffix if isinstance(suffix, str) else ""}'],
    df[f'{prefix if isinstance(prefix, str) else ""}_extension_{suffix if isinstance(suffix, str) else ""}'] = zip(*df[url_column].apply(lambda x: get_url_info(x, tlds)))
...
def get_url_info(url, tlds=tlds):
    ...
    -------
    tuple
        The URL normalized, apex, second, first, tld, path and extension

    Examples
    --------
    >>> get_url_info('https://www.google.com')
    ('www.google.com', 'www.google.com', 'www', 'google', 'com', None, None)
    >>> get_url_info('http://www.google.com')
    ('www.google.com', 'www.google.com', 'www', 'google', 'com', None, None)
    >>> get_url_info('www.google.com')
    ('www.google.com', 'www.google.com', 'www', 'google', 'com', None, None)
    >>> get_url_info('www.google.com/search/')
    ('www.google.com/search', 'www.google.com', 'www', 'google', 'com', 'search', None)
    >>> get_url_info('www.google.com/search.html?query=python')
    ('www.google.com/search.html?query=python', 'www.google.com', 'www', 'google', 'com', 'search.html', 'html')
    """
    ...
    url_normalized = get_url_normalized(apex, path)
    return url_normalized, apex, second, first, tld, path, extension
...
def is_valid_url(url):
    ...
    url_removed_protocol = remove_protocol(url)
    apex = get_apex(url_removed_protocol)
    _, first, tld = split_apex(apex)
    if url_removed_protocol is None or apex is None or first is None or tld is None or all(c.isalnum() or c == '-' for c in apex) == False:
        return False
    return True
